...
jQuery(document).ready(function($) {
	...
	tp.table = {
		...
		body_cells_pre:
			'<tr><td><span class="move-handle"></span></td><td><input type="checkbox" /><input type="hidden" class="visibility" name="table[visibility][rows][]" value="1" /></td>',
		body_cells_post: '<td><span class="move-handle"></span></td></tr>',
		body_cell: '<td><textarea rows="1"></textarea></td>',
		head_cell:
			'<th class="head"><span class="sort-control sort-desc" title="' +
			tablepress_strings.sort_desc +
			'"><span class="sorting-indicator"></span></span><span class="sort-control sort-asc" title="' +
			tablepress_strings.sort_asc +
			'"><span class="sorting-indicator"></span></span><span class="move-handle"></span></th>',
		foot_cell:
			'<th><input type="checkbox" /><input type="hidden" class="visibility" name="table[visibility][columns][]" value="1" /></th>',
		...
		unset_table_changed: function() {
			...
			).one('change', 'input, textarea, select', tp.table.set_table_changed)
		},
		change_id: function(/* event */) {
			...
			if (confirm(tablepress_strings.ays_change_table_id)) {
				...
				$('.table-shortcode')
					.val('[' + tablepress_options.shortcode + ' id=' + tp.table.new_id + ' /]')
					.click() // click() to focus and select
		...
		change_table_head: function(/* event */) {
			...
			$id('option-use-datatables')
				.prop('disabled', !tp.table.head)
				.change()
		...
		change_datatables: function() {
			...
			$datatables_checkbox
				.closest('tbody')
				.find('input')
				.not($datatables_checkbox)
				.prop('disabled', checkboxes_disabled)
		...
		prepare_ajax_request: function(wp_action, wp_nonce) {
			...
			table_visibility.rows = $table_body
				...
				.get()
			table_visibility.columns = $id('edit-form-foot')
				.find('input[type="hidden"]')
				.map(function() {
					if ('1' === $(this).val()) {
						return 1
		...
		preview: {
			trigger: function(/* event */) {
				...
				// validation checks
				if (
					$id('option-datatables-paginate').prop('checked') &&
					!/^[1-9][0-9]{0,4}$/.test($id('option-datatables-paginate_entries').val())
				) {
					alert(tablepress_strings.num_pagination_entries_invalid)
					$id('option-datatables-paginate_entries')
						.focus()
						.select()
					return
				}
				...
				$(this)
					.closest('p')
					.append(
						'<span class="animation-preview spinner is-active" title="' +
							tablepress_strings.preparing_preview +
							'"/>'
					)
				$('body').addClass('wait')
			...
			error: function(message) {
				$('.animation-preview')
					.closest('p')
					.after(
						'<div class="ajax-alert preview-error error"><p>' +
							tablepress_strings.preview_error +
							': ' +
							message +
							'</p></div>'
					)
	...
	tp.rows = {
		create: function(num_rows) {
			...
			return $(new_rows).each(function(row_idx, row) {
				$(row)
					.children()
					.filter(function(idx) {
						return -1 !== $.inArray(idx, column_idxs)
					})
		...
		insert: function(event) {
			var $selected_rows = $id('edit-form-body')
				.find('input:checked')
				.prop('checked', event.shiftKey)
				.closest('tr')
		...
		remove: function(/* event */) {
			var confirm_message,
				$selected_rows = $id('edit-form-body')
					.find('input:checked')
					.closest('tr')
		...
		move: {
			start: function(event, ui) {
				$(ui.placeholder)
					...
					.html(
						'<td colspan="' +
							(tp.table.columns + tp.table.no_data_columns_pre + tp.table.no_data_columns_post) +
							'"><div/></td>'
					)
		...
		sort: function() {
			var column_idx = $(this)
				...
				$head_rows = $table_body
					.find('.head-row')
					.prevAll()
					.addBack(),
				$foot_rows = $table_body
					.find('.foot-row')
					.nextAll()
					.addBack(),
				rows = $table_body
			...
			$.each(rows, function(row_idx, row) {
				row.sort_key = (
					...
					$(row)
						...
						.find('textarea')
						.val()
				).toLowerCase() // convert to string, and lower case for case insensitive sorting
			})
		...
		stripe: function(helper) {
			...
			helper = $(helper)
			var $rows = $id('edit-form-body')
				.children()
				.removeClass('odd head-row foot-row')
				.not(helper)
	...
	tp.columns = {
		append: function(/* event */) {
			...
			$id('edit-form-body')
				...
				.each(function(row_idx, row) {
					$(row)
						.children()
						.slice(-tp.table.no_data_columns_post)
		...
		insert: function(event) {
			var column_idxs,
				$selected_columns = $id('edit-form-foot')
					.find('input:checked')
					.prop('checked', event.shiftKey)
					.closest('th')
						...
						.each(function(idx, cell) {
							if (-1 !== $.inArray(idx, column_idxs)) {
								...
								$duplicated_cell
									.find('textarea')
									.removeAttr('id')
									.val($cell.find('textarea').val()) // setting val() is necessary, as clone() doesn't copy the current value, see jQuery bugs 5524, 2285, 3016
		...
		move: {
				...
				tp.columns.move.$cells = tp.columns.move.$rows
					...
					.each(function() {
						...
						tp.columns.move.$cell
							.insertAfter(tp.columns.move.$cell.nextAll().last())
							.clone()
							.addClass('move-hover')
							.insertAfter(tp.columns.move.$cell)
							.find('textarea')
							.val(tp.columns.move.$cell.find('textarea').val())
			...
			change: function(event, ui) {
				...
				tp.columns.move.$placeholder.each(function() {
					tp.columns.move.$cell = $(this)
					tp.columns.move.$cell.insertBefore(
						tp.columns.move.$cell
							.parent()
							.children()
							.eq(tp.columns.move.target_idx)
					)
				})
	...
	tp.cells = {
		...
		advanced_editor: {
			...
			keyopen: function(event) {
				...
				$advanced_editor.get(0).selectionStart = $advanced_editor.get(
					0
				).selectionEnd = $advanced_editor.val().length
			...
			save: function() {
				var $ve_content = $id('advanced-editor-content')
					.blur()
					.val()
				if (tp.cells.$textarea.val() !== $ve_content) {
					tp.cells.$textarea.val($ve_content)
					// position cursor at the end
					tp.cells.$textarea.get(0).selectionStart = tp.cells.$textarea.get(
						0
					).selectionEnd = tp.cells.$textarea.val().length
					tp.table.set_table_changed()
				}
		...
		checkboxes: {
			...
			multi_select: function(event) {
				...
				if (event.shiftKey) {
					...
					if (first_cb !== last_cb) {
						$checkboxes
							.slice(Math.min(first_cb, last_cb), Math.max(first_cb, last_cb))
	...
	tp.content = {
		...
		span: {
			...
			add: function(span) {
				var span_add_msg =
					'#rowspan#' === span ? tablepress_strings.rowspan_add : tablepress_strings.colspan_add
	...
	tp.reindex = function() {
		...
		if (tp.table.rows > 0) {
			tp.table.columns =
				$rows.first().children().length - tp.table.no_data_columns_pre - tp.table.no_data_columns_post
		} else {
			tp.table.columns = 0
		}
		...
		$rows
			.each(function(row_idx, row) {
				...
				$row.find('textarea')
					.val(function(column_idx, value) {
						...
						return value.replace(/([A-Z]+[0-9]+)(?::([A-Z]+[0-9]+))?/g, function(
							full_match,
							first_cell,
							second_cell
						) {
							...
							if (!known_references.hasOwnProperty(first_cell)) {
								...
								if ($cell.length) {
									known_references[first_cell] =
										tp.columns.number_to_letter(
											$cell.parent().index() - tp.table.no_data_columns_pre + 1
										) +
										($cell.closest('tr').index() + 1)
								} else {
							...
							if ('undefined' !== typeof second_cell && '' !== second_cell) {
								// Chrome and IE pass an undefined variable, while Firefox passes an empty string
								if (!known_references.hasOwnProperty(second_cell)) {
									$cell = $id('cell-' + second_cell)
									if ($cell.length) {
										known_references[second_cell] =
											tp.columns.number_to_letter(
												$cell.parent().index() - tp.table.no_data_columns_pre + 1
											) +
											($cell.closest('tr').index() + 1)
			...
			.each(function(row_idx, row) {
				// need a second loop here to not break logic in previous loop, that queries textareas by their old ID
				$(row)
					.find('textarea')
					.attr('id', function(column_idx /*, old_id */) {
		...
		$id('edit-form-head')
			.find('.move-handle')
			.html(function(idx) {
				return tp.columns.number_to_letter(idx + 1)
			})
	...
	tp.save_changes = {
		trigger: function(event) {
			...
			$(this)
				.closest('p')
				.append(
					'<span class="animation-saving spinner is-active" title="' +
						tablepress_strings.saving_changes +
						'"/>'
				)
		...
		ajax_success: function(data, status /*, jqXHR */) {
			if ('undefined' === typeof status || 'success' !== status) {
				tp.save_changes.error(
					'AJAX call successful, but unclear status. Try again while holding down the &#8220;Shift&#8221; key.'
				)
			} else if (
				'undefined' === typeof data ||
				null === data ||
				...
				tp.save_changes.error(
					'AJAX call successful, but unclear data. Try again while holding down the &#8220;Shift&#8221; key.'
				)
				...
				tp.save_changes.error(
					'AJAX call successful, internal saving process failed. Try again while holding down the &#8220;Shift&#8221; key.' +
						debug_html
				)
			} else {
		...
		ajax_error: function(jqXHR, status, error_thrown) {
			tp.save_changes.error(
				'AJAX call failed: ' +
					...
					' - ' +
					error_thrown +
					'. Try again while holding down the &#8220;Shift&#8221; key.'
			)
		},
		success: function(data) {
			...
			if (tp.table.id !== data.table_id) {
				...
				if ('pushState' in window.history && null !== window.history.pushState) {
					window.history.pushState(
						'',
						'',
						window.location.href.replace(/table_id=[0-9a-zA-Z-_]+/gi, 'table_id=' + data.table_id)
					)
				}
			}
			// update CSS class for data field form
			$id('edit-form')
				.removeClass('tablepress-edit-screen-id-' + tp.table.id)
				.addClass('tablepress-edit-screen-id-' + data.table_id)
			...
			if ($show_preview_buttons.length) {
				// check necessary, because Preview button might not be visible
				$show_preview_buttons.attr(
					'href',
					$show_preview_buttons
						.first()
						.attr('href')
						.replace(/item=[a-zA-Z0-9_-]+/g, 'item=' + data.table_id)
						.replace(/&_wpnonce=[a-z0-9]+/gi, '&_wpnonce=' + data.new_preview_nonce)
				)
			}
			...
			$('.animation-saving')
				...
				.after(
					'<div class="ajax-alert ' +
						div_class +
						'"><p>' +
						tablepress_strings['save_changes_' + type] +
						message +
						'</p></div>'
				)
	...
	tp.init = function() {
		var callbacks = {
				click: {
					...
					'#span-add-rowspan': function() {
						tp.content.span.add('#rowspan#')
					},
					'#span-add-colspan': function() {
						tp.content.span.add('#colspan#')
					},
		...
		$id('option-print-name')
			.change(tp.table.change_print_name_description)
			.change() // init dropdowns for name and description position
		$id('option-print-description')
			.change(tp.table.change_print_name_description)
			.change()
		...
		$('body').on(
			'focus',
			'.media-modal .media-frame-content input, .media-modal .media-frame-content textarea',
			function(event) {
				event.stopPropagation()
			}
		)
		...
		$id('edit-form-body').on(
			'click',
			'input:checkbox',
			{parent: '#edit-form-body'},
			tp.cells.checkboxes.multi_select
		)
		$id('edit-form-foot').on(
			'click',
			'input:checkbox',
			{parent: '#edit-form-foot'},
			tp.cells.checkboxes.multi_select
		)
...
;(function($) {
	$('#option-extra-css-classes').attr('pattern', '[A-Za-z0-9- _:]*')
	$('textarea[name^="table[data]"]').each(function() {
		const $el = $(this)
